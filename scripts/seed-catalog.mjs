...
if (!TABLE) {
  console.error('CATALOG_TABLE 환경변수가 필요합니다.');
  console.error('사용법: CATALOG_TABLE=테이블이름 node scripts/seed-catalog.mjs');
  process.exit(1);
}
...
async function seed() {
  for (const name of CATEGORIES) {
    await ddb.send(new PutCommand({ TableName: TABLE, Item: { PK: 'CATEGORY', SK: `CAT#${name}`, name } }));
  }
